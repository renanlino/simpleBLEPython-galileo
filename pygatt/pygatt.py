...
def reset_bluetooth_controller():
    ...
    subprocess.Popen(["hciconfig", "hci0", "down"]).wait()
    subprocess.Popen(["hciconfig", "hci0", "up"]).wait()

def lescan(timeout=5):
    scan = pexpect.spawn("hcitool lescan")
...
class BluetoothLeDevice(object):
    ...
    def connect(self, timeout=5.0):
        try:
            with self.connection_lock:
                self.con.sendline('connect ' + self.mac_address + ' random')
    ...
    def char_write(self, handle, value, wait_for_response=False, get_response=False):
        with self.connection_lock:
